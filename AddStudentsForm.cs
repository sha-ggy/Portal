...

        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand deleteEnrollmentsCmd = new SqlCommand("DELETE FROM Enrollments WHERE StudentID = @StudentID", con);
                    deleteEnrollmentsCmd.Parameters.AddWithValue("@StudentID", int.Parse(idDelTF.Text));
